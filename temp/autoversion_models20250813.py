...
- Scan models under env.model_dir_path (.meta_info)
...
def main():
    ...
    ap.add_argument("--behavior", choices=["auto","always","never"], default="auto",
                    help="When to run behavior hash (default: auto)")
    ap.add_argument("--include-names-in-schema", action="store_true",
                    help="Include tensor names in schema hash (default: off)")
    args = ap.parse_args()

    # validate .meta_info
    meta_info_path = os.path.abspath(args.meta_info)
    if not os.path.isfile(meta_info_path):
        print(json.dumps({"error": f".meta_info not found: {meta_info_path}"} , ensure_ascii=False))
        return

    meta = read_json(meta_info_path)
    project_root = os.path.dirname(meta_info_path)

    # models directory MUST come from .meta_info.env.model_dir_path
    env_cfg = meta.get("env", {})
    model_dir_cfg = (env_cfg.get("model_dir_path") or "").strip()
    if not model_dir_cfg:
        print(json.dumps({"error": "env.model_dir_path is missing or empty in .meta_info"}, ensure_ascii=False))
        return
    ...
    input_dir_cfg = meta.get("pipeline", {}).get("prediction", {}).get("input_dir_path", "") or ""
    ...
    for mpath in model_paths:
        ...
        if os.path.exists(reg_path):
            ...
            if args.behavior == "always":
                ...
                need_behavior = False
            else:  # auto: first-time or arch changed
