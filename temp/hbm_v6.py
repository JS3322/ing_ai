...
import pandas as pd
import tensorflow_model_optimization as tfmot  # 양자화 최적화를 위한 모듈
...
class HBMBandwidthModel:
    def __init__(self, train_csv='reference/train.csv', test_csv='reference/test.csv'):
        ...
        self.train_csv_path = os.path.join(self.project_root, train_csv)
        self.test_csv_path = os.path.join(self.project_root, test_csv)
        self.model = None
        self.X_train = None
        self.y_train = None
        self.X_test = None
    ...
    def create_quantized_model(self):
        """
        기본 모델을 생성한 후 양자화 인식 훈련을 적용한 모델을 생성합니다.
        """
        # 기본 모델 정의
        ...
        # 양자화 적용: 전체 모델에 대해 양자화 인식 훈련 적용
        quantize_model = tfmot.quantization.keras.quantize_model
        self.model = quantize_model(base_model)

        # 모델 컴파일
        self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001),
                           loss='mse',
                           metrics=['mae'])
        print("양자화 인식 훈련 모델 생성 및 컴파일 완료.")

    def train_model(self, epochs=100, batch_size=32):
        """
        학습 데이터를 이용하여 양자화 인식 훈련 모델을 학습합니다.
...
# 클래스 인스턴스 생성, 데이터 로드, 양자화 인식 모델 생성, 학습, 평가, 저장 예제
if __name__ == "__main__":
    check_gpu()  # GPU 사용 가능 여부 확인
    hbm_model = HBMBandwidthModel()
    hbm_model.load_data()
    hbm_model.create_quantized_model()  # 양자화 인식 모델 생성
